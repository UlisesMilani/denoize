...
const errorMessages: Record<string, { ja: string; en: string }> = {
  ...
  "evaluation.failed": { ja: "評価証跡を作成または検証できませんでした", en: "The evaluation evidence could not be created or verified" },
  ...
  "update.failed": { ja: "アプリ更新を完了できませんでした", en: "The application update could not be completed" },
...
const english: Record<string, string> = {
  "プロジェクト": "Project",
  "タイムラインプロジェクト": "Timeline project",
  "source fingerprint と presentation timebase に固定された manifest を検証し、同じ決定論的 assembler で部分区間を連結します。": "Validate a manifest bound to exact source fingerprints and presentation timebases, then join partial regions with the same deterministic assembler.",
  "プロジェクトルート": "Project root",
  "プロジェクト manifest": "Project manifest",
  "タイムライン": "Timeline",
  "manifest を選択": "Select a manifest",
  "参照を検証": "Verify references",
  "タイムラインを書き出す": "Export timeline",
  "WAV 出力": "WAV output",
  "署名付き実行証明（任意）": "Signed execution receipt (optional)",
  "出力前に exact plan を再計算します。manifest、source、既存出力は置換しません。": "The exact plan is recomputed before export. The manifest, sources, and existing outputs are never replaced.",
  "タイムラインを組み立てる": "Assemble timeline",
  "プロジェクト結果": "Project result",
  "manifest とプロジェクトルートを選んでください": "Select a manifest and project root",
  "オフライン bundle": "Offline bundle",
  "既定では manifest、設定、preset、参照、検証証跡だけを運びます。source 音声と model payload は上限付きで明示した場合だけ含めます。": "By default, the bundle carries only the manifest, settings, presets, references, and verification evidence. Source audio and model payloads are included only when explicitly enabled with bounds.",
  "Bundle 保存先": "Bundle destination",
  "source 音声を含める": "Include source audio",
  "明示した総量上限まで": "Up to the explicit aggregate limit",
  "model payload を含める": "Include model payloads",
  "署名と license を再検証": "Reverify signatures and licenses",
  "source 上限 MiB": "Source limit MiB",
  "model 上限 MiB": "Model limit MiB",
  "Bundle を作成": "Create bundle",
  "既存 bundle": "Existing bundle",
  "検査": "Inspect",
  "新規の取込先フォルダ": "New import destination folder",
  "指定": "Set",
  "Bundle を検証して取込": "Verify and import bundle",
  "評価証跡": "Evaluation evidence",
  "汎用音声復元": "Universal audio restoration",
  "署名済みBSRNN package v2だけを使い、雑音、残響、クリップ、帯域制限、codec歪み、packet loss、風雑音を一つの経路で復元します。安全ゲートに失敗した候補は破棄し、入力音声をそのまま出力します。": "Use only a signed BSRNN package v2 to restore noise, reverberation, clipping, bandwidth limitation, codec distortion, packet loss, and wind through one path. Candidates that fail a safety gate are discarded and the input audio is emitted unchanged.",
  "汎用復元入力": "Universal-restoration input",
  "汎用復元音声の保存先": "Universal-restoration destination",
  "署名付きBSRNN package v2": "Signed BSRNN package v2",
  "信頼済みMinisign公開鍵": "Trusted Minisign public key",
  "モデル種別": "Model family",
  "識別型（安全既定）": "Discriminative (safe default)",
  "Hybrid（実験）": "Hybrid (experimental)",
  "生成型（実験）": "Generative (experimental)",
  "レンダー役割": "Render role",
  "実験的alternateを許可": "Allow an experimental alternate",
  "Hybrid／生成型では明示的に必要": "Explicitly required for hybrid or generative models",
  "高度な安全ゲート": "Advanced safety gates",
  "最小劣化score": "Minimum degradation score",
  "最大energy増加 dB": "Maximum energy gain dB",
  "最大peak増加 dB": "Maximum peak gain dB",
  "最大新規clipping率": "Maximum new clipping ratio",
  "最大品質score低下": "Maximum quality-score regression",
  "汎用復元を実行": "Run universal restoration",
  "Clean入力は推論を迂回します。信号ゲートは単語・音素・話者同一性を証明しないため、生成型はPrimaryへ昇格できません。": "Clean input bypasses inference. Signal gates do not prove word, phoneme, or speaker identity fidelity, so generative models cannot be promoted to Primary.",
  "対象話者抽出": "Target-speaker extraction",
  "登録音声で指定した一人を混合音声から抽出します。対象不在、判定不確実、安全ゲート失敗では音声を一切保存せず、混合音声や別話者へフォールバックしません。": "Extract the one person identified by enrollment audio from a mixture. Target-absent, uncertain, and unsafe decisions save no audio and never fall back to the mixture or another speaker.",
  "混合音声": "Mixture audio",
  "対象話者の登録音声": "Target-speaker enrollment audio",
  "抽出音声の保存先": "Extracted-audio destination",
  "署名付き対象話者 package v2": "Signed target-speaker package v2",
  "package の Minisign 公開鍵": "Package Minisign public key",
  "署名付き promotion evidence": "Signed promotion evidence",
  "evidence の Ed25519 公開鍵": "Evidence Ed25519 public key",
  "高度な対象話者ゲート": "Advanced target-speaker gates",
  "最小 present 確率": "Minimum present probability",
  "最小 absent 確率": "Minimum absent probability",
  "採用時だけ混合音声からコピー": "Copy from mixture only when accepted",
  "対象話者を抽出": "Extract target speaker",
  "登録音声は生体情報です。推論直後に作業バッファをゼロ化し、PCM、埋め込み、digest、pathをレポートへ残しません。": "Enrollment audio is biometric data. Working buffers are zeroized immediately after inference, and no PCM, embedding, digest, or path is retained in the report.",
  "決定的音声修復": "Deterministic audio restoration",
  "クリップ、クリック、ハム、残響、風・破裂音を保守的に検出・修復し、完全な区間マスクと信頼度付きレポートを表示します。": "Conservatively detect and repair clipping, clicks, hum, reverberation, wind, and plosives, then display a complete interval mask and confidence-scored report.",
  "修復入力": "Restoration input",
  "修復音声の保存先（適用時は必須）": "Restored-audio destination (required when applying)",
  "保存しない": "Do not save",
  "クリップ波形を制約付きで復元": "Reconstruct clipped waveforms with constraints",
  "短いクリックを予測補間": "Predictively interpolate short clicks",
  "50/60 Hz 系の高調波を追跡": "Track harmonics of 50/60 Hz mains hum",
  "有限反復で後期残響を抑制": "Suppress late reverberation with bounded iterations",
  "低域バーストだけを局所補正": "Locally correct only low-frequency bursts",
  "WPEチャンネル処理": "WPE channel processing",
  "独立（各チャンネル）": "Independent (per channel)",
  "多チャンネル": "Multichannel",
  "検出のみ": "Detect only",
  "音声を書き換えずマスクを確認": "Inspect the mask without modifying audio",
  "修復を実行": "Run restoration",
  "検出・置換範囲、信頼度、処理別のエネルギー差を評価結果へ表示します。検出のみでは保存先を省略できます。": "The evaluation result shows detected and replaced ranges, confidence, and per-operation energy deltas. A destination is optional in detect-only mode.",
  "劣化診断・品質評価": "Degradation diagnosis and quality assessment",
  "先頭の有限区間を端末内で解析し、雑音、クリップ、ハム、クリック、残響、帯域、ドロップアウト、風雑音を信頼度付きで表示します。推定値だけで生成音声を採用しません。": "Analyze a bounded prefix locally and report noise, clipping, hum, clicks, reverberation, bandwidth, dropouts, and wind with confidence. Estimated values alone never authorize generated audio.",
  "候補／診断対象": "Candidate / diagnosis input",
  "比較元（任意）": "Baseline (optional)",
  "解析秒数": "Analysis seconds",
  "劣化を診断": "Diagnose degradation",
  "品質を評価・比較": "Assess / compare quality",
  "表示されるMOS proxyは人手MOSではありません。単語、音素、話者同一性、生成系のhallucinationは別の参照評価が必要です。": "The displayed MOS proxy is not human MOS. Words, phonemes, speaker identity, and generative hallucinations require separate reference evaluation.",
  "コーパス評価を実行": "Run corpus evaluation",
  "ライセンス、取得元 revision、前処理、SHA-256 を固定した manifest だけを実行し、品質・出力健全性・速度を署名付き JSON に保存します。": "Run only manifests that pin licenses, source revisions, preparation, and SHA-256, then save quality, output-integrity, and performance measurements as signed JSON.",
  "評価 manifest": "Evaluation manifest",
  "コーパスルート": "Corpus root",
  "人手評価結果（必要時）": "Human listening result (when required)",
  "評価証跡の保存先": "Evaluation evidence destination",
  "コーパスを検証": "Validate corpus",
  "署名付き評価を実行": "Run signed evaluation",
  "評価証跡を検証": "Verify evaluation evidence",
  "署名付き評価結果": "Signed evaluation result",
  "評価 manifest（任意）": "Evaluation manifest (optional)",
  "署名と manifest を検証": "Verify signature and manifest",
  "リグレッション比較": "Regression comparison",
  "両方の署名を認証し、同じ corpus・model・hardware・runtime・計測条件である場合だけ比較します。": "Authenticate both signatures and compare only when corpus, model, hardware, runtime, and measurement conditions match.",
  "ベースライン結果": "Baseline result",
  "候補結果": "Candidate result",
  "ベースライン公開鍵": "Baseline public key",
  "候補公開鍵": "Candidate public key",
  "比較を実行": "Compare",
  "評価結果": "Evaluation result",
  "manifest と corpus を選んでください": "Select a manifest and corpus",
  "DAW プラグイン": "DAW plug-in",
  "プラグイン契約": "Plug-in contract",
  "DAW が補正できる固定レイテンシと、オーディオコールバック内のゼロ allocation 契約を表示します。": "Shows the fixed latency that a DAW can compensate and the zero-allocation audio-callback contract.",
  "サンプルレート Hz": "Sample rate Hz",
  "プラグイン ID": "Plug-in ID",
  "報告レイテンシ": "Reported latency",
  "測定レイテンシ": "Measured latency",
  "CLAP 契約を確認しています。": "Checking the CLAP contract.",
  "レイテンシを再計測": "Measure latency again",
  "ニューラル契約": "Neural contract",
  "固定 GTCRN モデル、worker スケジューラ、遅延整合、過負荷 fallback を既存 DSP とは別のプラグイン ID で確認します。": "Inspect the pinned GTCRN model, worker scheduler, latency alignment, and overload fallbacks under a plug-in ID separate from the existing DSP.",
  "ニューラル ID": "Neural ID",
  "モデル ID": "Model ID",
  "ニューラル CLAP 契約を確認しています。": "Checking the neural CLAP contract.",
  "ニューラル遅延を再計測": "Measure neural latency again",
  "ポータブルプリセット": "Portable preset",
  "factory 設定を編集し、別の DAW や OS でも使える厳密な JSON 契約として読み書きします。": "Edit a factory setting and read or write it as a strict JSON contract that works across DAWs and operating systems.",
  "穏やか": "Gentle",
  "Factory を読込": "Load factory preset",
  "プリセットを読込": "Import preset",
  "プリセットを書出": "Export preset",
  "遅延を維持した dry 出力": "Dry output with latency preserved",
  "左右の gain reduction を共有": "Share gain reduction between left and right",
  "セッション復元": "Session restoration",
  "ポート構成と全 parameter を保存し、再読込時に同じ音声状態へ決定論的に戻します。": "Save the port configuration and every parameter, then deterministically restore the same audio state when reloaded.",
  "ポート構成": "Port configuration",
  "未選択時は no-clobber": "No-clobber when disabled",
  "セッションを読込": "Import session",
  "セッションを書出": "Export session",
  "未保存の編集内容です。": "These edits have not been saved.",
  "復元状態プレビュー": "Restoration-state preview",
  "DAW へ導入": "Install in a DAW",
  "リリース archive の denoize.clap を標準 CLAP フォルダへコピーし、DAW を再起動または plug-in rescan してください。": "Copy denoize.clap from the release archive to a standard CLAP folder, then restart the DAW or rescan plug-ins.",
  ...
  "アプリ更新": "Application updates",
  "署名マニフェストを確認": "Check signed manifest",
  "release channel、platform、SBOM、provenance、互換範囲、rollback policy を結ぶ署名を検証します。確認だけでは状態やインストールを変更しません。": "Verify the signature binding the release channel, platform, SBOM, provenance, compatibility range, and rollback policy. Checking alone changes neither state nor the installation.",
  "更新マニフェスト": "Update manifest",
  "マニフェスト署名": "Manifest signature",
  "署名付き更新を確認": "Check signed update",
  "公式リリースを確認": "Check official release",
  "ローカル署名を確認": "Check local signature",
  "復旧可能バンドル": "Recoverable bundle",
  "候補と last-known-good の成果物・SBOM・provenance を含む .dub を全バイト検証します。dry-run は読み取り専用です。": "Verify every byte of the .dub containing candidate and last-known-good artifacts, SBOMs, and provenance. Dry run is read-only.",
  "オフライン更新バンドル": "Offline update bundle",
  "認証済みバンドルを取得": "Download authenticated bundle",
  "明示的に適用": "Apply explicitly",
  "適用後は候補を pending-health とし、正常起動が確認されるまで last-known-good を削除しません。復旧時も anti-rollback floor は下げません。": "After apply, the candidate remains pending-health and last-known-good is retained until a healthy startup is confirmed. Recovery never lowers the anti-rollback floor.",
  "オフライン復旧": "Offline recovery",
  "ヘルス確認待ちの候補だけを破棄し、検証済み last-known-good を再び active にします。ネットワークやダウングレード例外は使いません。": "Discard only a candidate awaiting health confirmation and reactivate the verified last-known-good. This uses neither the network nor a downgrade exception.",
  "状態を再読込": "Refresh status",
  "Last-known-good へ復旧": "Recover last-known-good",
  "更新トランザクション": "Update transaction",
  "更新状態を読み込んでいます": "Loading update status",
  "実行版と管理状態が異なるため、検証済み管理版を再有効化しました": "Reactivated the verified managed version after detecting a runtime mismatch",
