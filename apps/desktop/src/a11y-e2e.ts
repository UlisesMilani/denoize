...
export async function runAccessibilityE2e(): Promise<void> {
  ...
  const tabPairsValid = tabs.length === 12 && tabs.every((tab) => {
    const panel = document.getElementById(tab.getAttribute("aria-controls") ?? "");
    return panel?.getAttribute("role") === "tabpanel"
      && panel.getAttribute("aria-labelledby") === tab.id;
  });
  check("runtime.tab-panel-pairs", tabPairsValid, "navigation tabs and panels do not form twelve valid pairs");
  check(
