...
if [[ $(grep -Fc "rustup toolchain install $release_toolchain " \
  "$repo_dir/.github/workflows/release.yml") != 8 ]]; then
...
if [[ $(wc -l < "$DENOIZE_FAKE_GH_LOG") != 23 ]]; then
  echo "offline verifier did not check the archive and all 22 artifacts" >&2
  exit 1
fi
