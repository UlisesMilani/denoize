...
if [[ $(grep -Fc "rustup toolchain install $release_toolchain " \
  "$repo_dir/.github/workflows/release.yml") != 9 ]]; then
...
if [[ $(wc -l < "$DENOIZE_FAKE_GH_LOG") != 25 ]]; then
  echo "offline verifier did not check the archive and all 24 artifacts" >&2
  exit 1
fi
